...
{
	name = "Reserved Slots", 
	author = "AlliedModders LLC & Edited: can4eyc | Саша Шеин  - vk.com/can4eyc", 
...
public void VIP_OnClientLoaded(int client, bool bIsVIP)
{
	...
	if (reserved > 0)
	{
		int clients = Client_GetCount(true, false, true);
...
int Client_GetCount(bool countInGameOnly = true, bool countFakeClients = true, bool onlyPublicPlayers = true)
{
	int numClients = 0;
	...
	for (int client = 1; client <= MaxClients; client++) {
		...
		if (onlyPublicPlayers && (flags = GetUserFlagBits(client), (flags & ADMFLAG_ROOT || flags & ADMFLAG_RESERVATION))) {
			continue;
		}
		if (onlyPublicPlayers && VIP_IsClientVIP(client)) {
			continue;
		}
		numClients++;
	}
...
{ 
	...
	if (GetClientCount(false) >= limit) {
		limit = GetClientCount(false);
	}
	

	
	sv_visiblemaxplayers.IntValue = limit + 1;
}
